...
class MyBoxLayout(BoxLayout):
    def __init__(self, **kwargs):
        ...
        single_doc_convtr_button = Button(text='Convert Single Doc', color=(1, 0.65, 0, 1), bold=True, italic=True)
        ...
        mltpl_doc_convtr_button = Button(text='Convert Multiple Docs', color=(1, 0.65, 0, 1), bold=True, italic=True)
        mltpl_doc_convtr_button.bind(on_press=self.on_mltpl_convtr_click)
        left_pane.add_widget(mltpl_doc_convtr_button)
        # Left pane (settings)
        settings_button = Button(text='Settings', color=(1, 0.65, 0, 1), bold=True, italic=True)
        settings_button.bind(on_press=self.on_settings_click)
        left_pane.add_widget(settings_button)
        # Left pane (utilities)
        utilities_button = Button(text='Utilities', color=(1, 0.65, 0, 1), bold=True, italic=True)
        utilities_button.bind(on_press=self.on_utilities_click)
        left_pane.add_widget(utilities_button)
        # Left Pane (history)
        history_button = Button(text='History', color=(1, 0.65, 0, 1), bold=True, italic=True)
